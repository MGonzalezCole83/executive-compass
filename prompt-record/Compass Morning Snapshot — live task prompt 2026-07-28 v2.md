...
> **Record copy of the live cloud scheduled task** "Compass Morning Snapshot" (8:30am Mon–Fri UK, push on completion). v2, 28 July 2026: Slack text digest added on Martin's sign-off — task-output images are not viewable from the iOS app, so each run now also posts a compact mechanical digest (mirror of the image, no judgement) to #all-executive-compass. Renders via the deployed code at Code/compass_mech/ — the image layout is code-governed, never improvised. Material changes are Protected under document 05.
