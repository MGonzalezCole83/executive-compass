...
> **This is a record copy, not the running instance.** Live version: cloud scheduled task "Compass Capture & Reconciliation" (hourly 8–4 UK + 6:00pm run, Mon–Fri, push on completion). Copy taken 26 July 2026 after the bundled Protected change approved with the Daily Briefing: PART 4 now writes decision-bearing replies to Daily Briefing output straight to Airtable, and chase drafts carry Martin's full signature block (Phonely number 01184 115010 — the 01189 number in older sent mail is dead). Replay suite green at time of change. Material changes to the live prompt are Protected changes under document 05 and must update this record copy in the same pass.
