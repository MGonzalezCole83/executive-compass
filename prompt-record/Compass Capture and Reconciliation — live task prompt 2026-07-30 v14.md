...
> **Record copy of the live cloud scheduled task** (push on completion). v14, 30 July 2026 — TRASH ANOMALY SWEEP added after the Ashton incident: Ashton Stone's 24 Jul GA-plan email was trashed (Outlook-side — Compass has no delete capability) and became structurally invisible to the inbox query; Martin caught it 29 Jul evening. New PART 1B queries in:trash newer_than:2d; person-mail found in Trash is flagged as a question ("accidentally deleted — restore and process?"), never auto-restored, never silently processed. The 24 Jul email itself was restored + Reviewed in the master thread 30 Jul; the Ipsley item carries the live state (GA plan is Martin's to send; his 29 Jul evening reply is in neither connected mailbox — location being confirmed). Prior same-week versions: v13 site diary, v12 chase pre-check, v11 reserve positions, v10 10am run. Byte-verified; replay suite passed.
