...
> **Record copy of the live cloud scheduled task** (push on completion). v12, 29 July 2026 — chase pre-check shipped after the John Russell cover-note incident (mechanical chase drafted against a stale item state: Notes/Chronicle knew the note was stood down, Title/Owed By didn't). Two mandatory checks before any chase draft: SUPERSESSION (read Notes + Chronicle; premise moved on → question, not draft) and STATED TIMELINE (counterparty's own stated timeline + 1 working day before any chase; re-date as evidence-based). AUTONOMY rule added: Title and Owed By update in the same write as Notes. v11 (29 Jul am): reserve-positions rule + expectation-reply handling. v10 (28 Jul): fifth run at 10am UK. Stored prompt byte-verified; replay suite passed.
